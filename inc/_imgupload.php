...
class imgUploder
{
	...
	function chk_Style()
	{
		...
		if ($this->fstyle=="image"){
			if (  $r!="jpg" && $r!="jpeg" && $r!="png" && $r!="gif" && $r!="svg" ){
		...
		if ($this->fstyle=="customer"){
			if ($this->fileex!="rar" && $this->fileex!="zip" && $this->fileex!="jpg" && $this->fileex!="jpeg" && $this->fileex!="doc" && $this->fileex!="docx" && $this->fileex!="xls" && $this->fileex!="xlsx" && $this->fileex!="pdf" && $this->fileex!="mp4"){
				$this->user_msg=$this->user_msg."檔案只支援rar,zip,jpg,pdf,doc,docx,xls,xlsx,mp4等格式<br>";
			}
		}
		if ($this->fstyle=="csv"){
			if ($this->fileex!="csv"){
	...
	function chk_Copy()
	{
		if (!move_uploaded_file($this->file,iconv("UTF-8", "big5", $this->file_dir.$this->file_name))) {
			//echo $this->file.'='.$this->file_dir.$this->file_name;
			$this->user_msg=$this->user_msg."檔案移動失敗!<br>".$this->file_error;
		}
	}
	...
	function createImg($target,$target_path,$target_image){
		...
		if (strtolower($this->fileex)=="gif"){
				...
				if (!imagepng($target,$targetfile,9)){
				$this->user_msg=$this->user_msg."縮圖建立失敗!<br>";
			}
		}
		else{
				if (!imagejpeg($target,$targetfile,100)){
				$this->user_msg=$this->user_msg."縮圖建立失敗!<br>";
			}
		}
	}
	...
	function createSmailImg($w,$h,$key,$target_path='',$target_image='',$smallnomake = false){ //建立小圖function
		...
		if($size[0]==$w && $size[1]==$h){
			copy($this->file_dir.$this->file_name, $this->file_dir.$this->file_sname.$this->file_name);
			return;
		}
