...
$col[] = array('column' => $colname['name'], 'name' => '會員名稱 ', 'order' => false, 'width' => '100');
$col[] = array('column' => 'platform_name', 'name' => '平台名稱 ', 'order' => false, 'width' => '100');
        ...
        $('#table1').coderlisthelp({
            debug: true, callback: function (obj, rows) {
                ...
                for (var i = 0; i < count; i++) {
                    ...
                    $tr.attr("onClick", "parent.closeBox(function(){parent.$('#myuser').html('"+row["<?php echo $colname['name']?>"]+"');parent.$('#mygame').html(' - "+row["platform_name"]+"');parent.$('#<?php echo $colname['member_id'];?>').val('"+row["<?php echo $colname['member_id']?>"]+"');parent.$('#<?php echo $colname['member_id'];?>').valid();parent.$('#<?php echo $colname['platform_id'];?>').val('"+row["<?php echo $colname['platform_id']?>"]+"');parent.$('#<?php echo $colname_t['platform_id'];?>').valid();})");
