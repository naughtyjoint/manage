...
$col[] = array('column' => $colname_ep['createtime'],'name' => $langary_Web_Manage_all['create_time'],'order' => false, 'width' => '80');
        ...
        $('#table1').coderlisthelp({
            debug: true, callback: function (obj, rows) {
                ...
                for (var i = 0; i < count; i++) {
                    ...
                    $tr.attr("editlink", "id=" + row["<?php echo $colname['id'];?>"] + "&pgram_id=" + row["<?php echo $colname_ep['id'];?>"]);
                    $tr.attr("delkey", row["<?php echo  $colname_ep['id'];?>"]);
                    $tr.attr("title",  " " + row["<?php echo $colname['name'];?>"] + " " + row["<?php echo $colname_ep['id'];?>"] + "號節目紀錄");
